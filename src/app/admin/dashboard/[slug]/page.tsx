...
function PageContent({ pageId, activeTab }: { pageId: string; activeTab: string }) {
  const { matchInfo, innings1, innings2, liveState, momentum } = useMatchData();
  const { community, castVote } = useMatchData();

  switch (pageId) {
    case 'home':
      return <HomeTab matchInfo={matchInfo} />;
    case 'live':
      if (activeTab === 'scorecard') return <ScorecardTab matchInfo={matchInfo} innings1={innings1} innings2={innings2} />;
      if (activeTab === 'analytics') return <AnalyticsTab innings1={innings1} innings2={innings2} liveState={liveState} momentum={momentum} />;
      return (
        <>
...
function AdminPageEditorInner() {
  ...
  // useAdminStore available for future sidebar integration

  // Derive pageId and pageData from slug — no useEffect needed
    ...
    <div className="min-h-screen bg-lux-bg">
      ...
      <div className="fixed top-0 left-0 right-0 z-50 bg-[#020204]/95 backdrop-blur-xl border-b border-[#1a1a22] px-4 py-2 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="w-px h-4 bg-[#1a1a22]" />
          <span className="text-[#F0EDE6] text-sm font-medium">{pageData.title}</span>
        </div>
      ...
      {/* Sub-tab navigation if page has tabs */}
      {tabs.length > 0 && (
        <div className="fixed top-[40px] left-0 right-0 z-40 bg-lux-card border-b border-lux-border">
          <div className="max-w-7xl mx-auto overflow-x-auto scrollbar-none flex">
            {tabs.map((tab) => (
              <Link
                key={tab.id}
                href={`/admin/dashboard/${slug}${tab.id === tabs[0]?.id && !tabParam ? '' : `/${tab.id}`}`}
      ...
      {/* Live page content with admin edit context */}
      <div className={tabs.length > 0 ? 'pt-[80px]' : 'pt-[40px]'}>
        <div className="max-w-7xl mx-auto px-3 sm:px-5 lg:px-8 py-4 sm:py-6 lg:py-8">
          <PageContent pageId={pageData.id} activeTab={activeTab} />
        </div>
        <Footer />
      </div>
